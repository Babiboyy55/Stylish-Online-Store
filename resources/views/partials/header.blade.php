<!-- SVG symbol sprite (icons used across the site) -->
          ...
          <ul id="navbar" class="navbar-nav fw-bold justify-content-end align-items-center flex-grow-1">
            <li class="nav-item dropdown">
              ...
              <ul class="dropdown-menu fw-bold">
                <li>
                  <a href="{{ url('/') }}" class="dropdown-item">Trang chủ V1</a>
                </li>
                <li>
                  <a href="{{ url('/') }}" class="dropdown-item">Trang chủ V2</a>
                </li>
              </ul>
            </li>
            <li class="nav-item">
              <a class="nav-link me-5" href="{{ url('/men') }}">Nam</a>
            </li>
            <li class="nav-item">
            ...
            <li class="nav-item">
              <a class="nav-link me-5" href="{{ route('sale') }}">Giảm giá</a>
            </li>
          </ul>
        </div>
      </div>
      <div class="user-items ps-0 ps-md-5">
          ...
          @auth
            @if(auth()->user()->is_admin)
              <li class="pe-3">
                <a href="{{ url('/admin') }}" class="btn btn-sm btn-danger" title="Admin Panel">
                  <svg width="16" height="16" fill="currentColor" class="me-1" style="vertical-align: text-bottom;">
          ...
          <li class="pe-3">
            @if(auth()->check())
              <a href="{{ url('account') }}" class="border-0" title="Tài khoản">
            @else
              <a href="{{ url('login') }}" class="border-0" title="Đăng nhập">
            @endif
          ...
          <li class="pe-3 position-relative">
            <a href="{{ url('/cart') }}" class="border-0 position-relative d-inline-block" aria-label="View cart">
...
<div class="search-box bg-dark position-relative collapse" id="search-box" style="z-index: 9999;">
  <div class="search-wrap p-4" style="position: relative; z-index: 10000;">
    <div class="close-button">
      <button class="btn btn-close btn-close-white" id="search-close-btn" aria-label="Close"></button>
    </div>
    <form id="search-form" class="text-lg-center text-md-left pt-3" action="{{ url('/search') }}" method="get">
      <input name="q" id="search-input-field" type="text" class="search-input" placeholder="Tìm kiếm..." autocomplete="off">
  ...
  <script>
    (function(){
      ...
      document.addEventListener('DOMContentLoaded', function(){
        ...
        if (closeButton && searchBox) {
          closeButton.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            console.log('Close button clicked!'); // Debug log
            
            // Simply remove the 'show' class
